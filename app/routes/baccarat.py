...
GET    /api/v1/baccarat/sessions/me          fetch the active session
POST   /api/v1/baccarat/sessions/me/play     place bets, deal a round
DELETE /api/v1/baccarat/sessions/me          end the current session
...
def _resolve_caller():
    ...
    if sess is None:
        return None, False, _err("no baccarat session", "NO_SESSION", 404)
...
def play_route():
    ...
    try:
        rnd, payouts = play_round(sess, bets)
    except BaccaratError as e:
        return _err(str(e), "BACCARAT_ERROR", 409)
    return jsonify(round=rnd.to_dict(), payouts=payouts, total_profit=sum(payouts))
